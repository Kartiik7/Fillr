...
const sendVerificationEmailAsync = async (email, verifyUrl) => {
  ...
  console.log('[Verify] Sending verification email to %s', masked);
  ...
  try {
    ...
    if (result.error) {
      console.error('[Verify] Resend API error for %s — %s', masked, result.error.message);
    } else {
      console.log('[Verify] Email sent to %s — id: %s', masked, result.data?.id);
    }
  } catch (err) {
    console.error('[Verify] Email delivery failed for %s — %s', masked, err.message);
  }
};
...
exports.sendVerificationEmail = async (user) => {
  ...
  sendVerificationEmailAsync(user.email, verifyUrl).catch((err) => {
    console.error('[Verify] Unexpected error in async email:', err.message);
  });
};
...
exports.resendVerification = async (req, res, next) => {
  try {
    ...
    sendVerificationEmailAsync(email, verifyUrl).catch((err) => {
      console.error('[Verify] Unexpected error in async resend:', err.message);
    });
  } catch (err) {
    next(err);
  }
